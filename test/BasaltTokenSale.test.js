...
contract("BasaltTokenSale", ([owner, referrer, user, user1]) => {
    ...
    before(async () => {
        ...
        const firstUnlockAmtBP = "0";
        this.basaltTokenSale = await BasaltTokenSale.new(this.basaltToken.address, [paymentToken], firstUnlockAmtBP, { from: owner });
    ...
    it('withdrawUnlockedBasaltTokens', async () => {
        ...
        const perPeriod = "357.142857142857142857";//10000 / 28
        assert.equal(web3.utils.fromWei(await this.basaltTokenSale.getAllowedAmount(user)), "257.142857142857142857");//perPeriod -100
        assert.equal(web3.utils.fromWei(await this.basaltTokenSale.getAllowedAmount(user1)), perPeriod);//10000 / 28
        ...
        assert.equal(web3.utils.fromWei(await this.basaltToken.balanceOf(user).valueOf()), perPeriod);
        assert.equal(web3.utils.fromWei(await this.basaltToken.balanceOf(user1).valueOf()), perPeriod);

        await time.increase(STAGE_DURATION);// 1
        assert.equal(web3.utils.fromWei(await this.basaltTokenSale.getAllowedAmount(user)), perPeriod);
        assert.equal(web3.utils.fromWei(await this.basaltTokenSale.getAllowedAmount(user1)), perPeriod);
        await this.basaltTokenSale.withdrawUnlockedBasaltTokens({ from: user });
        await this.basaltTokenSale.withdrawUnlockedBasaltTokens({ from: user1 });
        assert.equal(web3.utils.fromWei(await this.basaltToken.balanceOf(user).valueOf()), "714.285714285714285714");
        assert.equal(web3.utils.fromWei(await this.basaltToken.balanceOf(user1).valueOf()), "714.285714285714285714");
